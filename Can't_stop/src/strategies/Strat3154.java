...
public class Strat3154 implements Strategie {
    ...
    private int[] earn = {1,2,3,3,4,5,4,3,3,2,1};
    ...
    */
    private int changeGain(int[] avancement, int[] avancementStart){
        int gain = 0;

        // incrémentation des gains réaliser sur ce tour 
        for(int i = 0; i<11; i++){
            if(((avancement[i] - avancementStart[i]) >= earn[i]) && ((avancement[i] - avancementStart[i]) <= earn[i]*2)){
                gain += 1;
            }else if(((avancement[i] - avancementStart[i]) >= earn[i]*2) && ((avancement[i] - avancementStart[i]) <= earn[i]*3)){
                gain += 2;
            }else if((avancement[i] - avancementStart[i]) >= earn[i]*3){
                gain += 3;
            }
        }
    ...
    */
    public int choix(Jeu j) {
        ...
        if(nbCoup <= 0){
            avancementStart = j.avancementJoueurEnCours();
        }
    ...
    */
    public boolean stop(Jeu j) {
        ...
        if(((j.getBonzesRestants() <= 0) && (changeGain(avancement, avancementStart) >= 3)) || (stop)){
            return true;
        }else{
            return false;
        }
